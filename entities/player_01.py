    ...
    def collide(self, xvel, yvel, up, down, left, right, platforms):
        ...
        for p in platforms:
            if pygame.sprite.collide_rect(self, p):
                ...
                if xvel > 0 and not isinstance(p, StairsBlock):
                    self.rect.right = p.rect.left
                if xvel < 0 and not isinstance(p, StairsBlock):
                    self.rect.left = p.rect.right
                if yvel > 0 \
                        and (
                        not isinstance(p, StairsBlock)
                        or (isinstance(p, StairsBlock) and not self.onStairs and not down)
                ):
                    self.rect.bottom = p.rect.top
                    self.yvel = 0
                if yvel < 0 and not isinstance(p, StairsBlock):
