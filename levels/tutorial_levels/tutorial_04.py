...
def show_image(screen, width, height):
    ...
    temp = rescale_image_height(
        "images/wedding/rewards/end_level_animation_tutorial_02.png",
        height,
        diff=diff
    )
    ...
    temp = rescale_image_height(
        "images/wedding/carlos_ruiz_pozo/carlos_ruiz_pozo_02.png",
        height,
        diff=0.3
    )
    screen.blit(
    ...
    pg_print_message(screen, "Segunda barra!", int(round(width / 5)), int(round(height / 6)), size=64)
...
def tutorial_04():
    level = Level("levels/tutorial_levels/tutorial_04.txt", 64, velocity_jump=6)
